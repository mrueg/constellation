...
func (t *TFIDF) Embed(_ context.Context, docs []Doc) (*Space, error) {
	...
	for _, term := range sortedKeys(mapToFloat(df)) {
		...
		words = append(words, cand{term, t.idfOf(n, c)})
	}
	// Prefer rare-but-not-unique terms; ties broken alphabetically so the whole
	// pipeline stays deterministic.
	byIDF := func(c []cand) {
		sort.SliceStable(c, func(i, j int) bool {
			if c[i].idf != c[j].idf {
				return c[i].idf > c[j].idf
			}
			return c[i].term < c[j].term
		})
	}
	byIDF(words)
