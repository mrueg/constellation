...
func (c *ListsClient) Lists(ctx context.Context) ([]List, error) {
	...
	for {
		...
		next := reply.Viewer.Lists.PageInfo
		// A cursor that does not move is not progress: re-sending the same
		// request would loop forever on a reply claiming another page without
		// saying where it starts.
		if !next.HasNextPage || next.EndCursor == "" || next.EndCursor == cursor {
			return out, nil
		}
