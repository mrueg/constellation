...
// The outlier test drops a repository only when it is both statistically
// anomalous for its category and substantially worse than a typical member.
// The second half was expressed as a multiple of the median, which inverts
// when the median is negative — and cosines in the reduced space do go below
// zero. There the floor rose above the median, so the guard stopped existing
// precisely for the least coherent categories it was written to protect.
func TestRelativeFloorIsBelowTheMedianInBothDirections(t *testing.T) {
	for _, med := range []float64{0.9, 0.5, 0.36, 0.01, -0.01, -0.1, -0.42, -0.9} {
		got := relativeFloor(med)
		if got > med {
			t.Errorf("median %+.2f: floor %+.3f is above the median, so the guard cannot bind", med, got)
		}
		// It must also stay a guard rather than becoming the whole test: the
		// gap is proportional to the magnitude.
		if want := math.Abs(med) * (1 - outlierRelativeGuard); math.Abs((med-got)-want) > 1e-12 {
			t.Errorf("median %+.2f: gap %.4f, want %.4f", med, med-got, want)
		}
	}
}

// Where the median is positive — the ordinary case — the fix must change
// nothing, or it would be a re-tuning wearing a bug fix's clothes.
func TestRelativeFloorUnchangedForPositiveMedians(t *testing.T) {
	for _, med := range []float64{0.9, 0.5, 0.36, 0.2, 0.01, 0} {
