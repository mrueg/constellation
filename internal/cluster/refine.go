...
// relativeFloor is the "substantially worse than typical" half of the outlier
// test: how far below the median a fit has to be before it counts as bad
// regardless of the spread.
//
// It is expressed as a fraction of the median's magnitude rather than as a
// multiple of the median itself. Those agree wherever the median is positive,
// which is the ordinary case — but a category can have a negative median fit,
// since cosines in the reduced space run below zero, and there the naive form
// inverts: 0.6 times a negative number is *greater* than that number, so the
// floor rose above the median and the guard silently stopped existing exactly
// for the least coherent categories it was written to protect.
func relativeFloor(med float64) float64 {
