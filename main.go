...
func (a *applyFlags) flags() []cli.Flag {
	base := []cli.Flag{
		...
		&cli.BoolFlag{Name: "adopt", Usage: "take over an existing list whose name matches a category even if this tool did not create it; without this such a list is left untouched", Destination: &a.adopt},
		...
		&cli.BoolFlag{Name: "reconcile", Usage: "make the account match the plan instead of only adding to it: repositories leave categories they no longer belong to and dropped categories are deleted, touching only lists this tool created", Destination: &a.reconcile},
